...
このアプリは **2つのデータベースシステム** を連携させています：
...
**重要なファイル**:
- `src/lib/supabase/server.ts`: サーバー側の認証クライアント（API内で使用）
...
`.env.local` に以下を設定（`.env.local` は `.gitignore` 済み、GitHubにプッシュされない）:
...
**なぜ DATABASE_URL と DIRECT_URL の2つが必要？**
...
**ポイント**:
- ユーザーA は「送信ボタンを押した瞬間」に画面更新（楽観的更新）
- ユーザーB は「1秒以内」に自動で画面更新（Realtime）
- エラーが起きた場合のみ、ユーザーAにエラーメッセージ表示
...
優先順位順に記載（詳細は CONTEXT_HANDOVER_*.md を参照）:
